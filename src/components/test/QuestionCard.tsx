...
export function QuestionCard({
  ...
  return (
    <div className="w-full bg-white py-6 px-3 sm:py-12 sm:px-8 rounded-2xl sm:rounded-3xl border border-[var(--color-hazzi-gray-300)] shadow-sm flex flex-col items-center justify-center">
      <h2 className="text-base sm:text-lg md:text-xl font-[family-name:var(--font-ibm-plex)] font-bold text-center text-[var(--color-hazzi-ink)] mb-6 sm:mb-12 max-w-2xl break-keep leading-snug sm:leading-relaxed">
        {order}. {question.text}
      </h2>

      <div className="flex items-center justify-between w-full max-w-2xl sm:gap-4">
        <span className="hidden sm:block text-rose-500 font-bold text-sm md:text-base w-24 text-right">
          전혀 아니다
        </span>
        
        <div className="flex items-center justify-between sm:justify-center w-full sm:w-auto px-1 sm:px-0 gap-1.5 sm:gap-6">
          <span className="sm:hidden text-rose-500 font-bold text-[11px] xs:text-xs whitespace-nowrap shrink-0">
            전혀 아니다
          </span>
          {scores.map((score) => {
            ...
            return (
              <button
                ...
                className={clsx(
                  "rounded-full border-2 transition-all duration-200 hover:scale-110 active:scale-95 cursor-pointer shrink-0",
                  styles.size,
                  styles.color
                )}
          ...
          <span className="sm:hidden text-blue-500 font-bold text-[11px] xs:text-xs whitespace-nowrap shrink-0">
            매우 그렇다
          </span>
        </div>
